	...
	<div class="container">
		...
		if (!@$conexion) {
			...
			if (!$rec) {
				...
				echo "<tr class='active'><th>Devolución</th><th>F.Emisión</th><th>RUC</th><th>Cliente</th><th>Pedido</th><th>TOTAL</th><th ></th><th ></th></tr> \n";
				while ($obj = sqlsrv_fetch_object($rec)) {
					echo "<tr class='info' id='" . $obj->IDDOCUMENTO . "'><th>" . $obj->PEDIDO . "</button></th><td>" . $obj->FECHAEMISION . "</td><td>" . $obj->RUC . "</td><td>" . $obj->NOMBRECLIEPROV . "</td><td>" . $obj->REFE . "</td><td>" . number_format($obj->TOTAL, 2, '.', '') . "</td><td style='display:none'>" . $obj->DIRECCION . "</td>
								<td style='display:none'>" . $obj->PEDIDO . "</td>
								<td><button type='button' class='buttontabla2' id='detalle' 
								data-toggle='modal' data-target='#myModal' onclick='load(" . $obj->IDDOCUMENTO . ")''><span class='glyphicon glyphicon-list-alt'></span></td>
								<td><button type='button' class='buttontabla2' id='eliminacion-devulocion' ><span class='glyphicon glyphicon-trash'></span></td></tr> \n";
				}
				echo "</table></div></div>\n";
				/*
