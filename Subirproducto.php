...
<div class="container">
		<?php
					...
					if(!@$conexion){
						...
						$sql ="SELECT A.Codigo as Codigo,A.Descripcion as Item ,A.PrecioVenta as PrecioVenta FROM TBITEMS A  
						...
						$rec = sqlsrv_query($conexion,$sql,$params);
							if (!$rec) {
								...
								echo "<div class='panel'><div class='table-responsive'><table class='table table-hover' style='width:100%' > \n";
						      	echo "<tr class='active'><td>Código</td><td>Descripción</td><td>Precio</td></tr> \n";
						      	while( $obj = sqlsrv_fetch_object( $rec)) {
						      	echo "<tr class='info'><td id='codigo'><a class='l1s'</a>".$obj->Codigo."</td><td>".$obj->Item."</td><td>".$obj->PrecioVenta."</td></tr> \n";
								} 
								echo "</table></div></div>\n";
					/*
