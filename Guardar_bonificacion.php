	...
	echo "No hay conexion";
		...
		{	
			...
			foreach($xml->session as $item){
				...
				$seridocref ="001";
				///doc referencia
					$sqlNUMEROREF="SELECT TOP 1 * FROM tbDocumentos 
									WHERE PERIODO = YEAR(GETDATE()) AND MES =MONTH(GETDATE()) AND IDDIARIO =89 
									AND Descripcion='PROFORMAS' order by 1 desc";
					$queryNUMEROREF = sqlsrv_query($conexion,$sqlNUMEROREF);
					$corrNUMEROREF=sqlsrv_fetch_array($queryNUMEROREF,SQLSRV_FETCH_ASSOC);
					//
				$nuedocref =$corrNUMEROREF['NumeDocu'];
				$DIRECCIONCLIEPROV =$_POST['valorCaja4'];
...
/*/						estudiomercado.asistente gmail.com
...
*/
				...
				echo"<script type=\"text/javascript\">alert('el numero de pedido es:  $numerodoccc '); </script>";  
			}//echo"<script type=\"text/javascript\">alert('el numero de bonificacion es:  $numerodoccc ');window.location='Menu.php'; </script>"; 
		}
