			...
			if(!@$conexion){
				...
				$idUsuario2=$filavendedor['idClieProv'];
					...
					if ($rowpropiedades['valor']=='S') {
						...
						DOC.DireccionClieProv as DIRECCION  FROM TBDOCUMENTOS
						...
						AND TBDOCUMENTOS.SERIDOCU= ? AND TBDOCUMENTOS.NUMEDOCU= ? AND DOC.NUMEDOCU LIKE '%' + ? + '%' AND TBDOCUMENTOS.IDDIARIO=89 AND DOC.ESTADO='R' AND TBDOCUMENTOS.DESCRIPCION ='CONSOLIDADO PEDIDOS' AND DOC.IDTIPODOCU=215 order by 1 desc";
					}
					else{
						...
						WHERE RUC LIKE '%' + ? + '%' AND NOMBRECLIEPROV LIKE '%' + ? + '%' AND IDDIARIO=89 
				...
				foreach( $errors as $error ) {
								...
								echo "<div class='panel panel-primary'><div class='table-responsive'><table class='table table-hover' style='width:100%' > \n";
						      	echo "<tr class='active'><td>Pedido</td><td>F.Emisión</td><td>RUC</td><td>Cliente</td><td>TOTAL</td><td ></td></tr> \n";
						      	while( $obj = sqlsrv_fetch_object( $rec)) {
						      	echo "<tr class='info'><td><button type='submit' class='buttontabla' id='NUMERAL' >
							<span class='glyphicon glyphicon-upload'></span>".$obj->PEDIDO."</button></td><td>".$obj->FECHAEMISION."</td><td>".$obj->RUC."</td><td>".$obj->NOMBRECLIEPROV."</td><td>".number_format($obj->TOTAL, 2, '.', '')."</td><td style='display:none'>".$obj->DIRECCION."</td>
								<td style='display:none'>".$obj->PEDIDO."</td>
								<th><button type='button' class='buttontabla' id='detalle' 
								data-toggle='modal' data-target='#myModal' onclick='validar(".$obj->PEDIDO.")'><span class='glyphicon glyphicon-thumbs-down'></span> Devolver todo <span class='glyphicon glyphicon-thumbs-down'></span></button></th></tr> \n"; 
								} 
								echo "</table></div></div>\n";
					/*
