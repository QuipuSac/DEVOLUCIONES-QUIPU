...
<head>
	...
	<script>
		$(document).ready(function() {
			console.clear();
			$('#resultado').on('click', '#eliminacion-devulocion', function() {
				console.clear();
				__this = $(this).closest('tr');
				var $info = __this.attr('id');
				
				console.log($info);
				alertify.confirm("confirmacion", "seguro que desea eliminar la devolucion?",
					function() {
						$.ajax({
							type: "post",
							url: 'GuardarDevolucionesQuipu.php',
							data: {
								'accion': 'eliminar devolucion',
								'data': $info
							},
							dataType: "json",
	...
	<div class="container">
		...
		<div class="panel bordepanel">
			<div class="panel-heading" STYLE="background-color:#6E6E6E">
				<font color="FFFFFF">
					<h3 class="panel-title">Buscador de Devoluciones</h3>
				</font>
			</div>
					...
					<form role="form" method="post">
						<div class="row">
							...
							<div class="col-sm-3">
								...
								<input id="sericonsolidado" type="number" class="form-control" placeholder="Clave/Serie" readonly="true" value=<?php echo date("Y") . $_POST['idvendedorpedido']  ?>>
							</div>
							<div class="col-sm-3">
								<span class="input-group-addon" id="valor2">Número:</span>
								<input id="numconsolidado" type="number" class="form-control" placeholder="Consolidado pedido" onClick="this.select()">
							</div>
							<div class="col-sm-2">
								<span class="input-group-addon">Nro.Pedido:</span>
								<input id="Pedido" type="number" class="form-control" placeholder="Nro.pedido" onClick="this.select()">
							</div>
						...
						<div class="row">
							...
							<div class="col-sm-4">
								...
								<input id="ruc" type="text" class="form-control" placeholder="Doc.Cliente" onClick="this.select()">
							</div>
							<div class="col-sm-4">
								<span class="input-group-addon" id="valor5">Cliente:</span>
								<input id="cliente" type="text" class="form-control" placeholder="Raz.Social" onClick="this.select()">
							</div>
						</div><br>
					</form>
					<div class="col-sm-offset-2 col-sm-10">
						<div>
							<button type="submit" class="buttonpepdido" onclick="buscarPedido($('#ruc').val(), $('#cliente').val(), $('#sericonsolidado').val(), $('#numconsolidado').val(), $('#Pedido').val());return false;" value="Calcula">
						...
						<form method="POST" action=<?php echo $ventanpedido; ?>>
							...
							<div class="col-xs-2">
								<input class="form-control" type="text" id="PERFILUSUARIO" placeholder="PERFILUSUARIO" name="PERFILUSUARIO" style="visibility:hidden" value="<?php echo $PERFILUSUARIO; ?>">
							</div>
						</form>
					</div>
				</form>
